...
if length(fn) < 2*K+1
    ...
    error('Wählen Sie bitte ein kleineres K')
...
for i=1:length(a_k)
    ...
    a_k(i) = (2/length(fn))*sum(fn.*reshape(cos((i-1)*2*pi*n/length(fn)),length(n),1));
...
for j = 2:length(b_k)
    ...
    b_k(j) = (2/length(fn))*sum(fn.*reshape(sin((j-1)*2*pi*n/length(fn)),length(n),1));
